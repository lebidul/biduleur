...
def process_files(extraction=False, postprocessing=False):
            if extraction:
                ...
                for file in files:
                    document_url = file['webContentLink']
                    file_name = file['name']

                    local_file_path = os.path.join(TMP_DIRECTORY, f"{file_name}")
                    if not os.path.exists(local_file_path):
                        # Télécharger le fichier localement pour l'analyse
                    ...
                    if text.strip():
                        ...
                        log_processing(file_name, document_url, os.path.basename(output_file_path), "Text Extraction")
                    else:
                        ...
                        log_processing(file_name, document_url, os.path.basename(output_file_path), "OCR")
